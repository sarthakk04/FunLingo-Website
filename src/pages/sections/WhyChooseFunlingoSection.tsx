...
const progressItems = [
    {
      ...
      description: "Learn words and phrases in the context of real conversations.",
      progress: "95%",
    },
    {
      title: "Fun & Engaging",
      ...
      <section className="flex flex-col w-full items-center gap-14 p-24 bg-backgroundbackdrop scroll-animate">
        ...
        <div className="flex w-full max-w-[1008px] items-start gap-8">
              ...
              {progressItems.map((item, index) => (
                <div key={index} className="flex flex-col gap-3 w-full">
                  <div className="flex justify-between items-center">
                    <h4 className="font-heading-h6 text-textwhite">{item.title}</h4>
                    <span className="font-body-small-medium text-brandlime-green">{item.progress}</span>
                  </div>
                  <p className="font-body-normal-regular text-textbody">{item.description}</p>
                  <div className="w-full bg-gray-700 rounded-full h-2.5">
                    <div className="bg-[linear-gradient(135deg,rgba(2,91,207,1)_0%,rgba(0,145,189,1)_100%)] h-2.5 rounded-full" style={{ width: item.progress }}></div>
                  </div>
                </div>
              ))}
            </CardContent>
          ...
          <div className="flex-1">
             <img src="/figmaAssets/hero-image-placeholder.png" alt="Funlingo in action" className="rounded-2xl w-full h-full object-cover" />
          </div>
        </div>
      </section>
    );
